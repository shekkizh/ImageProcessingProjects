...
image = cv2.cvtColor(cv2.imread(args["image"]), cv2.COLOR_BGR2HSV)
background = cv2.cvtColor(cv2.imread(args["background"]), cv2.COLOR_BGR2HSV)
...
background_gray = cv2.equalizeHist(cv2.cvtColor(background, cv2.COLOR_BGR2GRAY))
output = cv2.absdiff(image_gray, background_gray)
thresh, output = cv2.threshold(output, 0, 255, cv2.THRESH_BINARY + cv2.THRESH_OTSU)

image_processed = np.ma.array(image.copy(), mask = [output,output,output])
...
print std
output = cv2.inRange(image, mean - 2*std, mean + 2*std)
...
cv2.imshow("input", cv2.cvtColor(image, cv2.COLOR_HSV2BGR))
